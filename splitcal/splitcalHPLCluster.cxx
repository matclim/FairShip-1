...
void splitcalHPLCluster::Clusterize(){
	...
	for(auto hit:_vectorOfHits){
		if(hit->IsUsed()==0)	Seedhit=hit;
	}	
	std::vector<splitcalHit*> *clustervector= new std::vector<splitcalHit*>;
	for(int i=0;i<_vectorOfHits.size();i++){
		if(_vectorOfHits.at(i)->IsUsed()==0 && TMath::Sqrt((Seedhit->GetX()-_vectorOfHits.at(i)->GetX())*(Seedhit->GetX()-_vectorOfHits.at(i)->GetX())+(Seedhit->GetY()-_vectorOfHits.at(i)->GetY())*(Seedhit->GetY()-_vectorOfHits.at(i)->GetY())<10)){
			clustervector->push_back(_vectorOfHits.at(i));
		}
	}

	vectorOfClusters.push_back(clustervector);
...
void splitcalHPLCluster::ComputeEtaPhiE()
{
...
/*regression splitcalHPLCluster::LinearRegression(std::vector<double >& x, std::vector<double >& y) {
...
void splitcalHPLCluster::SetStartPoint(splitcalHit*& h) {
  SetStartPoint(h->GetX(),h->GetY(),h->GetZ());
} 

void  splitcalHPLCluster::SetEndPoint(splitcalHit*& h) {
